// server.js
// Install deps:
//   npm i express cors compression duckdb dotenv
//
// Run locally:
//   SRC_DB=./cbb_data.duckdb node server.js
...
const SRC_DB =
  process.env.SRC_DB ||
  process.env.src_db ||
  path.join(process.cwd(), "cbb_data.duckdb");
...
// Convert BigInt -> Number recursively
...
// Mount AI agent router under /api/agent
...
try {
  if (!fs.existsSync(SRC_DB)) {
    console.warn(
      `[warn] DuckDB file not found at ${SRC_DB}. Starting without DB connection.`,
    );
  } else {
    console.log(`[info] Using DuckDB at ${SRC_DB}`);
    db = new duckdb.Database(SRC_DB, duckdb.OPEN_READONLY);
    conn = db.connect();

    const cpuCount = Math.max(1, os.cpus()?.length || 1);
    const memForDuck = Math.max(
      256 * 1024 ** 2,
      Math.floor(os.totalmem() * 0.7),
    );
    const bytes = (n) => `${n}B`;
    ...
    conn.run(
      `PRAGMA temp_directory='${path.join(os.tmpdir(), "duckdb_tmp")}'`,
    );

    console.log(
      `[info] DuckDB initialized with ${cpuCount} threads and ~${memForDuck} bytes`,
    );
  }
} catch (e) {
  console.error("[error] Failed to initialize DuckDB:", e);
  // Still start the server so you can deploy and attach DB later
  db = null;
  conn = null;
}
...
// Play by play (team-level)
app.get("/api/game/:gameId/pbp", async (req, res) => {
  const gameId = numberOrNull(req.params.gameId);
  if (gameId === null) {
    return res.status(400).json({ error: "Invalid gameId" });
  }
  ...
  try {
    ...
    console.error("pbp query error", e);
    res.status(500).json({ error: "pbp query failed" });
  }
});

// Shot chart
app.get("/api/game/:gameId/shots", async (req, res) => {
  const gameId = numberOrNull(req.params.gameId);
  if (gameId === null) {
    return res.status(400).json({ error: "Invalid gameId" });
  }
  ...
  try {
    ...
    console.error("shots query error", e);
...
app.listen(PORT, () => {
  ...
  console.log(`[info] DB path configured as: ${SRC_DB}`);
});
